...
(function ($) {
    var isNewUser = false;
    var hasYZM = false;
    var InvitedBy = $.getQueryStringByName("c");
    var jumpUrl = "/e/registcash/index.html" + "?c=" + InvitedBy;
    $.alertF("活动奖励升级啦，请参加新活动！", "确定", function () {
        ...
        function doRegister() {
            var regProcess = function () {
                ...
                $.AkmiiAjaxPost("/StoreServices.svc/Anonymous/luckdrawregister", data, false).then(function (d) {
                    if (d.result) {
                        $.alertF("恭喜,注册成功!<br/>抽中红包金额：" + d.amount.toFixed(2) + "元!", "确定", function () {
                            if (opt.regCallback)
                                opt.regCallback(d);
                            else {
                                window.location.replace("/html/product/index.html");
