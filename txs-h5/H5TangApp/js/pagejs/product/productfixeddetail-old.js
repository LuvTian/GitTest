...
var getCurrentProduct = function () {
    ...
    $.AkmiiAjaxPost(url, data, true).then(function (data) {
        if (data.result) {
            ...
            $("#product-detail").attr("href", "/html/product/regular-product-detail-old.html?id=" + product.productid);
    ...
    if (product.countdownsecond > 0) {
        ...
        if (product.type == 99 && !product.newuser) {
            ...
            if (product.paytype == 2) {
                $("#product-buy").attr("href", "/html/product/productfixedbuy.html?id=" + product.productid + "&paytype=" + 2).find("#product-buy-link").text("立即抢购(仅支持至尊宝购买)");
            }
            else {
                $("#product-buy").attr("href", "/html/product/productfixedbuy.html?id=" + product.productid).find("#product-buy-link").text("立即抢购");
            }
            //特殊渠道必须先买新手标
