...
$(function () {
    ...
    (function (Product) {
        function getProductList(request, success, needLoder, error, complete) {
            var url = "/StoreServices.svc/product/getproductsecondlist";
    ...
    var ProductFixedListPage = (function () {
        ...
        ProductFixedListPage.prototype.getProductList = function (lastId, accountResponse, sort) {
            var topDom = this;
        ...
        ProductFixedListPage.prototype.initSoldOut = function (product) {
            ...
            htmlArray.push("<div class='tagsContainer'>");
            //添加tag  <span class='tag{0}'>{1}</span>
            htmlArray.push("</div>");
            ...
            switch (product.status) {
                case 6:
                    ...
                    htmlArray.push("<div class='sell_out_ed'></div>");
                    break;
                case 8:
                    htmlArray.push("<div class='productSellProcessing'><span class='pspBar'><i></i></span><span class='pspPercent'>已借" + process + "%</span></div>")
                    break;
                case 5:
                    htmlArray.push("<div class='productSellProcessing'><span class='pspBar'><i></i></span><span class='pspPercent'>已借" + process + "%</span></div>")
                    break;
        ...
        ProductFixedListPage.prototype.initSOling = function (product, accountResponse) {
            ...
            htmlArray.push(" <p class='productName'>" + product.title + "</p>");
            if (product.taglist && product.taglist.length) {
                htmlArray.push("<div class='product_label'>");
                //添加tag  <span class='tag{0}'>{1}</span>
                $.each(product.taglist, function (k, v) {
                    if(v.type=="1"){
                        htmlArray.push("<i class='label_desc' style='color:" + v.tagcolor + ";border-color:" + v.tagcolor + "'>" + v.title + "</i>");
                    }else if(v.type=="2"){
                        htmlArray.push("<i class='label_desc' style='color:#fff;background-color:" + v.tagcolor + "'>" + v.title + "</i>");
                    }
                })
                htmlArray.push("</div>");
            }
            ...
            if (product.isreservation) {
                htmlArray.push("<div class='bookingproduct'></div>");
            }
            htmlArray.push("<div class='productSellProcessing'><span class='pspBar'><i></i></span><span class='pspPercent'>已借" + process + "%</span></div>")
            if (product.istailed && product.surprisemessage.length) {
                htmlArray.push("<div class='surprise'>");
                htmlArray.push(product.surprisemessage);
                htmlArray.push("<a href='/html/product/productfixedbuy-new.html?surprised_btn_click=true&id=" + product.productid + "'>抢购<i class='leftArrow'/></a>");
                htmlArray.push("</div>");
            }
            htmlArray.push("</li>");
    ...
    $("#productSortContainer li").click(function () {
        ...
        pageindex = 1;
        temp = 0;
        productFixedList.getProductList("", "", true);
    })



    $("#mainBody").append('<div class="bottom"><img src="' + $.resurl() + '/css/img2.0/productfixed_bottomimg.png" class="bottom_pic"/></div>');
});