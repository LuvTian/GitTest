...
var productBuy = function(pass, returnurl) {
    ...
    */
    ...
    $.AkmiiAjaxPost(url, data).then(function(data) {
        if (data.result) {
            if (paytemp == 1) {
                var paramurl = "/html/paycenter/operation-success.html?type=buy&profittime=" + data.date + "&amount=" + $product_buy_amount.val() + "&product=" + encodeURIComponent('至尊宝') + "&title=" + encodeURIComponent('投资成功') + "&buybank=" + encodeURIComponent(accounttext) + "&iswithholdauthoity=" + account.iswithholdauthoity + "&returnurl=" + success_returnurl;
            } else if (paytemp == 4) {
                var paramurl = "/html/paycenter/operation-success.html?type=buy&profittime=" + data.date + "&amount=" + $product_buy_amount.val() + "&product=" + encodeURIComponent('至尊宝') + "&title=" + encodeURIComponent('投资成功') + "&buybank=" + encodeURIComponent("银行卡") + "&returnurl=" + success_returnurl;
            }
            ...
            $.alertNew(data.errormsg, null, function() {
                ...
                if (account.iswithholdauthoity == 3) {
                    $.PaymentHtmlNew($("#product-buy-amount").val(), "", function(password) {
                        $.closePWD();
                        productBuy(password);
                    }, cancel, ""+accounttext+"", account.basicbalance);
                } else {
                    //收银台模式
                    var returnurl = window.location.href;
                    productBuy("", returnurl);
    ...
    if (basictemp) {
        if (account.basicbalance >= amount) {
            $.PaymentHtmlNew(amount, "", function(password) {
                $.closePWD();
                productBuy(password);
            }, cancel, ""+accounttext+"", account.basicbalance, paymethod);
            paytemp = 1;
