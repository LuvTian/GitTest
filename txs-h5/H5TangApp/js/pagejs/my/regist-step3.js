    ...
    $BindBtn.bind("click", function () {
        ...
        $.AkmiiAjaxPost("/StoreServices.svc/user/bindcardadvance", params).then(function (d) {
            ...
            if (d.result) {
                ...
                _gsq.push(["T", "GWD-002985", "track", "/targetpage/binding_success"]);
                if (account.invitedby == _CHANNELCODE) {
                    _MVbindCard();
                }
                var returnurl=$.getQueryStringByName("returnurl");
                var skip_sina_process=$.getQueryStringByName("skip_sina_process");
                if(returnurl && skip_sina_process){
                    window.location.replace(decodeURIComponent(returnurl));
                    return;
                }
                //新浪二期改造流程
                if (account.iswithholdauthoity == 0) {
                    var _returnurl = window.location.origin + "/eback.html?r=" + window.location.origin;
                    var result = $.showActiveSinaAccount(_returnurl, currentDate, account.referralcode, account.iscashdesknewuser);
                    if (!result) {
                        if(returnurl){
                            window.location.replace(decodeURIComponent(returnurl));
                        }
...
var getUserInfo = function () {
    ...
    $.AkmiiAjaxPost(url, {}, true).then(function (data) {
        if (data.result) {
            ...
            $.removeLS(account.referralcode + "showasinaupgradedate");
             //是否同意协议
            if (data.accountinfo.issignmoneyboxandhtffund) {
               $(".depositbtntext").html("转入");
            } else {
                 $(".depositbtntext").html("充值");
            }
            switch (data.accountinfo.customstatus) {
                case 0:
                    if (window.location.pathname != "/html/my/regist-step1.html") {
                        window.location.replace("/html/my/regist-step1.html?returnurl="+returnurl+"&skip_sina_process="+skip_sina_process);
                    }
                    break;
                case 1:
                    if (window.location.pathname != "/html/my/regist-step2.html") {
                        window.location.replace("/html/my/regist-step2.html?returnurl="+returnurl+"&skip_sina_process="+skip_sina_process);
                    }
                    break;
                case 2:
                    if (window.location.pathname != "/html/my/regist-step3.html") {
                        window.location.replace("/html/my/regist-step3.html?returnurl="+returnurl+"&skip_sina_process="+skip_sina_process);
                    }
                    break;
                case 3:
                    if (returnurl) {
