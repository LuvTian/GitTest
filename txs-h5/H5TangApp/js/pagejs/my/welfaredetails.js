...
//开始时间
var startTime = decodeURIComponent( $.getQueryStringByName( "startTime" ) );
      ...
      if ( d.result ) {
        if ( d.couponactivity != null ) {
          ...
          if ( startTime != "" && startTime != undefined && startTime != "null" ) {
            startTime = startTime.substring( 0, 10 ).replace( /-/g, "." );
          } else {
            startTime = startTime
          }
          var endTime = overduetime.substring( 0, 10 ).replace( /-/g, "." );
          if ( status == 5 ) {
            textstatus = "已消费";
          ...
          if ( d.couponactivity.isusetemplate && d.couponactivity.coupontemplates ) {
            ...
            item += '<p class="userinfo az-clearfix bb pt0 "><span class="line-l az-left gray-font" style="width:75%;">' + d.couponactivity.couponabstract + '</span>';
          ...
          if ( textstatus == "已消费" || textstatus == "已过期" || textstatus == "待生效" ) {
            item2 += '<p class="indent">订单编号：' + orderid + '</p><p class="indent-time">下单时间：' + wintime + '</p></div><div class="small-4 columns az-padding0 az-text-right p-right-7"><span class="consume col-f-bord">' + textstatus + '</span></div>';
          } else {
            item2 += '<p class="indent">订单编号：' + orderid + '</p><p class="indent-time">下单时间：' + wintime + '</p></div><div class="small-4 columns az-padding0 az-text-right p-right-7"><span class="consume">' + textstatus + '</span></div>';
          }
          item2 += '</div></div></div>';
          ...
          if ( startTime != "" && startTime != "undefined" && startTime != "null" ) {
            item3 += '<div class="row userinfo pt0"><div class="small-6 fl">使用期限：</div><div class="small-6 fl tr">' + startTime + "-" + endTime + '</div></div>';
          } else {
            item3 += '<div class="row userinfo pt0"><div class="small-6 fl">领用有效期至：</div><div class="small-6 fl tr">' + overduetime + '</div></div>';
          }
...
var selectTemplate = function( data ) {
  ...
  switch ( coupontemplates.TemplateType ) {
    case 1: //满减券
      ...
      arr.push( '<p class="expired-font fz-16rem">领用有效期至：' + ( data.activityvalidend ) + '</p>' );
      arr.push( '</div>' );
