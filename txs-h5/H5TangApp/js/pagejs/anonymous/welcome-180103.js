...
function productIntroduce( ajaxurl ) {
  ...
  $.AkmiiAjaxGet( ajaxurl, true ).then( function( d ) {
    if ( d.code == 200 ) {
      ...
      if ( _selectedproductinfo.type == 6 ) {
      ...
      if ( _data.userType == 1 || _data.userType == 2 ) {
        ...
        $( ".wel_right .wel_rate" ).find( "z" ).text( _selectedproductinfo.rate );
        $( ".wel_left .wel_day" ).find( "z" ).text( _selectedproductinfo.duration );
        if ( d.data.tagList ) {
          $.each( d.data.tagList, function( index, item ) {
            $( ".product_label" ).eq( 1 ).append( "<span>" + item.title + "</span>" ).css( {
              "color": item.tagColor,
              "border-color": item.tagColor
            } );
          } )
        }
      ...
      $( ".wel_mainpush" ).on( "click", function() {
        if ( d.data.selectedProductInfo.type == 6 ) {
      ...
      $.each( d.data.preferredProductList, function( i, item ) {
        ...
        tmllist += tmpl.replace( "{0}", item.productName || "" )
          ...
          .replace( "{7}", ( !!item.minAmount ? item.minAmountSuffix : "" ) )
          .replace( "{8}", item.type == 6 ? "jqsJump_clk('4','" + item.productId + "')" : "window.location.href=jumpUrl(" + item.type + "," + "'" + item.productId + "'" + "," + "'" + item.productName + "'" + "," + null + ")" )
          .replace( "{9}", ( !!_strsplit[ 0 ] ? "inline-block" : "none" ) )
          .replace( "{10}", item.rateActivity + "%" )
          .replace( "{11}", !!item.rateActivity ? "block" : "none" )
          .replace( "{12}", tagHand( item.tagList ) )
      ...
      if ( _data ) {
        if ( _data.recordList.length == 0 ) {
          ...
          $.each( _data.recordList, function( inx, ite ) {
            ...
            $( ".rewardlist li" ).eq( inx ).find( "img" ).attr( "src", ite.mainImgUrl )
            ...
            $( ".rewardlist li" ).on( "click", function() {
