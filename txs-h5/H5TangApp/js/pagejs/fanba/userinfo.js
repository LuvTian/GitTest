      ...
      if (data.result) {
        ...
        if (account.customstatus <= 1) {
          ...
          if (account.customstatus == 2) {
            ...
            $("#my-card").attr("href", "/html/fanba/mybankcard.html");
...
var withholdauthoity = function(account) {
  $("#my-HandleWithholdAuthority").click(function() {
    if (account.customstatus < 3) {
      $.confirmF("您的个人安全资料还未完善，现在去完善吧", null, null, null, $.RegistSteplinkFanba);
      return false;
    }
    ...
    if (account.iswithholdauthoity == 0) {
      ...
      returnurl = window.location.origin + "/eback.html?r=" + window.location.origin + "/Html/fanba/UserInfo.html?t=99";
