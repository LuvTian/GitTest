        ...
        if (data.result) {
            ...
            if (data.isglobalmaintenance) {
                ...
                $("#deposit").click(function () {
                    window.location.href = "/Html/fanba/user-deposit.html?returnurl="+window.location.href;
                    /*暂时去掉*/
                    //if (account.customstatus < 3) {
                ...
                $("#withdraw").click(function () {
                    window.location.href = "/Html/fanba/user-withdraw.html?returnurl="+window.location.href;
                    //if (account.customstatus < 3) {
                    //    $.alertF("您的资料还未完善，现在去完善吧", null, $.RegistSteplink);
                    //    return;
                    //}
...
var BindCurrentData = function () {
    ...
    $.AkmiiAjaxPost(url, {}, true).then(function (data) {
        if (data.result) {
            ...
            if (i > levelArry.length - 1) { i = levelArry.length - 1; }
...
var getMyMitteilung = function () {
    ...
    $.AkmiiAjaxPost(url, {}, true).then(function (data) {
        if (data.result && data.accountchatinfo.newmsgcount > 0 && data.accountchatinfo.islook) {
            $('.wxicon.icon').find("i:eq(0)").show();
            $('.wxicon.icon').find("i:eq(0)").text(data.accountchatinfo.newmsgcount);
            $("#myicon .wxicon").append('<i></i>');
        }
        else if (data.result && data.accountchatinfo.newmsgcount > 0 && !data.accountchatinfo.islook) {
            $('.wxicon.icon').find("i:eq(0)").show();
            $('.wxicon.icon').find("i:eq(0)").text(data.accountchatinfo.newmsgcount);
        }
        else {
