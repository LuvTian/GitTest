...
var welfare = {
    ...
    physical_tpl: '<li onclick="welfare.jumpToDetail(\'{0}\');{5}">\
					...
					<div class="exchangeAmount">投资满<span>{3}</span> <b>{4}</b></div>\
				</li>',
    virtual_tpl: '<li class="virtual" onclick="welfare.jumpToDetail(\'{0}\');{7}">\
					...
					<div class="exchangeAmount">投资满<span>{5}</span> <b>{6}</b></div>\
				</li>',
    renderPhysicalHtml: function (data) {
        ...
        //     inventory = "售罄";
        // }
        var limit = (data.amountThreshold / 10000) > 1 ? data.amountThreshold / 10000 + "万元" : data.amountThreshold + "元";
        limit += "+" + (data.originPoint - data.discount) + "唐果";
        temp += tpl.format(
            url,
            data.imgUrl || "",
            data.name || "",
    ...
    getData: function () {
        if (!this.loadingData) {
            ...
            $.AkmiiAjaxGet(apiUrl_prefix + "/promotion/discount/current?id=" + userId + "&page=" + this.page + "&size=" + this.size, false).then(function (d) {
